<template>
  <div class="quiz">
    ...
    <main class="body" v-if="!finished">
      <transition :name="slideDir" mode="out-in"><div class="dossier" :key="currentIdx">
        <template v-if="!isBonus">
          <div class="dos-head"><span class="dos-q mono">反差情境 {{ currentIdx + 1 }}</span><span class="dos-lvl mono">场景选择</span></div>
          <h2 class="dos-question">{{ currentQ.question }}</h2>
          <div class="picks">
            <button v-for="(opt, i) in currentQ.options" :key="opt.id" class="pick" :class="{ 'is-pick': selected === i }" @click="selectOption(i)">
        ...
        <template v-else>
          ...
          <button class="bonus-cta" @click="submitBonus"><Icon icon="mdi:file-hidden" /> 生成反差档案</button><button class="bonus-back" @click="goPrev">再想一下</button>
        </template>
      </div></transition>
    ...
    <div class="finish" v-else><div class="finish-ring"><Icon icon="mdi:file-hidden" /></div><h2>31 项已完成</h2><p class="mono">正在整理你的反差人格档案…</p></div>
  </div>
</template>
<script setup>
...
const bonusMood = computed(() => { if (bonusScore.value <= 2) return '较一致'; if (bonusScore.value <= 4) return '轻反差'; if (bonusScore.value <= 6) return '有切换'; if (bonusScore.value <= 8) return '反差明显'; return '高反差' })
...
const totalQuestions=questions.length+1; const isBonus=computed(()=>currentIdx.value>=questions.length)
...
function submitBonus(){ const answerList=questions.map(q=>answersMap.value[q.id]).filter(Boolean); if(answerList.length!==questions.length)return; const objectiveScore=answerList.reduce((sum,a)=>sum+(a.score||0),0); const total=objectiveScore+bonusScore.value; finished.value=true; setTimeout(()=>router.push({path:'/result',query:{objective:objectiveScore,bonus:bonusScore.value,total,answers:JSON.stringify(answerList)}}),1000) }
</script>
<style scoped>
