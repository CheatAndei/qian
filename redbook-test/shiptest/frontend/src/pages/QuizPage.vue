<template>
  <div class="quiz">
    ...
    <main class="body" v-if="!finished">
      <transition :name="slideDir" mode="out-in">
        <div class="card" :key="currentIdx">
          ...
          <p class="card-no mono">信号采集 {{ pad(currentNo) }}</p>
    ...
    <footer class="monitor-band" :class="{ 'is-react': flash }" v-if="!finished">
      ...
      <div class="mb-info">
        <div class="mb-top">
          <span class="mono mb-label"><span class="mb-dot"></span>回波监测 · 信号采集中</span>
          <span class="mono mb-sig">信号 {{ dispSig }}%</span>
        </div>
        <div class="mb-bar">
...
.monitor-band {
  ...
  background: linear-gradient(180deg, rgba(54, 241, 166, 0.015), rgba(54, 241, 166, 0.06));
  padding: 10px 16px calc(10px + env(safe-area-inset-bottom));
  transition: background 0.4s ease, border-color 0.4s ease;
}
.monitor-band.is-react {
  border-top-color: var(--accent);
  background: linear-gradient(180deg, rgba(54, 241, 166, 0.06), rgba(54, 241, 166, 0.14));
  animation: bandKick 0.6s ease;
}
@keyframes bandKick {
  0% { box-shadow: inset 0 14px 26px -14px rgba(54, 241, 166, 0); }
  22% { box-shadow: inset 0 14px 26px -10px rgba(54, 241, 166, 0.4); }
  100% { box-shadow: inset 0 14px 26px -14px rgba(54, 241, 166, 0); }
}
.mb-radar {
  width: 58px;
  height: 58px;
...
.opt.is-pick {
  ...
  box-shadow: 0 0 0 1px var(--accent) inset, 0 6px 18px rgba(54, 241, 166, 0.16);
}
.opt.is-pick .opt-key {
  background: var(--accent);
  color: #04140d;
...
@keyframes finishPulse {
  0%, 100% { box-shadow: 0 0 0 0 rgba(54, 241, 166, 0.3); }
  50% { box-shadow: 0 0 0 14px rgba(54, 241, 166, 0); }
}
.finish h2 {
  font-size: 22px;
  color: var(--ink);
