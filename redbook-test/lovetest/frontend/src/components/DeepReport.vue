<template>
  <section class="dr">
    <div class="dr-head">
      <span class="dr-kicker mono">DEEP SCAN</span>
...
const props = defineProps({
  ...
  dims: { type: Object, required: true }, // {rush, anxiety, ...}
  accent: { type: String, default: '#FF3B6B' },
  product: { type: String, default: 'lovetest' },
})
...
.dr-lock {
  ...
  text-align: center;
  padding: 24px;
  background: radial-gradient(120% 80% at 50% 40%, rgba(11, 14, 20, 0.55), rgba(11, 14, 20, 0.86));
  border-radius: var(--radius);
}
.dr-lock-icon {
  font-size: 34px;
...
.dr-lock-btn {
  ...
  font-weight: 800;
  cursor: pointer;
  box-shadow: 0 10px 30px rgba(255, 59, 107, 0.34);
}
.dr-lock-btn:active {
