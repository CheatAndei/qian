    ...
    <section class="convert">
      ...
      <button class="convert-btn" @click="goNext">
        ...
        <span class="convert-tx">
          ...
          <small>亲密度看完了，再测测你俩到底合不合。</small>
        </span>
        <Icon class="convert-arr" icon="mdi:chevron-right" />
      </button>
    </section>

    <MoreTests current="lvtest" />
...
const nextProductUrl = import.meta.env.VITE_NEXT_PRODUCT_URL || 'https://cp.xpytt.com'
function goNext() {
...
.act.share {
  color: #1a0410;
  box-shadow: 0 8px 24px rgba(255, 122, 162, 0.24);
}
.act.retake {
...
.sh-card {
  position: relative;
  background: linear-gradient(180deg, #231922, #171116);
  border: 1px solid var(--line-strong);
  border-radius: 20px;
