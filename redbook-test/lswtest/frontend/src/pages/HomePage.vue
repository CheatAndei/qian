<template>
  <div class="hp weather">
    ...
    <header class="hp-status rise" style="animation-delay: 40ms">
      ...
      <span class="hp-dev mono">LSW · 气象站</span>
      <span class="hp-sig"><Icon icon="mdi:access-point" /></span>
    </header>
    ...
    <section class="hp-stage rise" style="animation-delay: 130ms">
      <div class="hp-stage-head">
        <span class="mono">关系气象观测</span>
        <span class="mono hp-stage-code">CH · ATTACH</span>
      </div>
      <WeatherField />
    </section>
    ...
    <!-- 实时读数带 -->
    <div class="hp-stats">
      <div class="hp-stat rise" style="animation-delay: 230ms"><b class="mono">{{ statA }}</b><em>依恋天气</em></div>
      <div class="hp-stat rise" style="animation-delay: 290ms"><b class="mono">{{ statB }}</b><em>观测项</em></div>
      <div class="hp-stat rise" style="animation-delay: 350ms"><b class="mono">3<small>′</small></b><em>约需时长</em></div>
    </div>
    ...
    <main class="hp-hero">
      ...
      <p class="rise" style="animation-delay: 560ms">
        20 个亲密关系场景，把你的本能反应摊开看一眼。不是给关系下结论，只是看见你常用的那套天气系统。
      </p>
    </main>
...
import WeatherField from '../components/WeatherField.vue'

const router = useRouter()
// 默认即为终值，保证任何情况下都可见；JS 在位时做计数动画
const statA = ref(4)
const statB = ref(20)
...
/* 入场：从下方淡入，结束停在可见态；reduced-motion 下全局把时长归零→瞬间到可见 */
