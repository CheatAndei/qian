...
restoreSelected()
function selectOption(i) { if (lock) return; lock = true; selected.value = i; const opt = currentQ.value.options[i]; readoutText.value = opt.read || ''; reactTo(opt.score, true); answersMap.value[currentQ.value.id] = { score: opt.score, dims: opt.dims || {}, text: opt.text, read: opt.read, drain: opt.drain || null }; setTimeout(() => goNext(), 520) }
