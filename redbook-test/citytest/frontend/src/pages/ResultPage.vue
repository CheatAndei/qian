<template>
  <div class="result" v-if="report">
    <header class="r-status">
      <span class="rec"><i></i> 目的地生成</span>
      <span class="mono code">偏好 {{ coord }}</span>
    </header>
    ...
    <section class="signal-sec">
      <div class="gauge">
        ...
        <div class="gauge-center">
          ...
          <span class="gauge-unit">城市匹配度 %</span>
        </div>
      </div>
    </section>
    ...
    <section class="convert">
      ...
      <button class="convert-btn" @click="goNext">
        ...
        <span class="convert-tx">
          <strong>情侣亲密度等级测试</strong>
          <small>城市找到了，再测测你们到底亲到哪一级。</small>
        </span>
    ...
    <p class="disclaimer">本测试由 AI 生成，仅供娱乐参考，不构成现实选择或专业建议。</p>
      ...
      <div v-if="showShare" class="sh-overlay" @click="showShare = false">
        <div class="sh-modal" @click.stop>
          <div class="sh-card" ref="shareCardRef">
            ...
            <div class="sh-head">
              <span class="rec"><i></i> CITY MAP</span>
              <span class="mono">偏好 {{ coord }}</span>
            </div>

            <!-- 静态迷你城市图，html2canvas 友好 -->
            <div class="sh-radar">
            ...
            <p class="sh-kicker mono">城市磁场测试 · 指数 {{ report.signal }}%</p>
            ...
            <div class="sh-foot">
              ...
              <div class="sh-brand">
                <p>小红书搜「城市磁场测试」</p>
                <p class="sh-brand-sub mono">看看哪座城市最像你</p>
              </div>
            </div>
          </div>
          <div class="sh-acts">
...
.act.share {
  color: #071913;
  box-shadow: 0 8px 24px rgba(82, 214, 182, 0.24);
}
.act.retake {
...
.sh-card {
  position: relative;
  background: linear-gradient(180deg, #162119, #101611);
  border: 1px solid var(--line-strong);
  border-radius: 20px;
