    ...
    <main class="body" v-if="!finished">
      <transition :name="slideDir" mode="out-in">
        <div class="card" :key="currentIdx">
          ...
          <p class="card-no mono">城市偏好 {{ pad(currentNo) }}</p>
...
.monitor-band {
  ...
  background: linear-gradient(180deg, rgba(82, 214, 182, 0.015), rgba(82, 214, 182, 0.06));
  padding: 10px 16px calc(10px + env(safe-area-inset-bottom));
  transition: background 0.4s ease, border-color 0.4s ease;
}
.monitor-band.is-react {
  border-top-color: var(--accent);
  background: linear-gradient(180deg, rgba(82, 214, 182, 0.06), rgba(82, 214, 182, 0.14));
  animation: bandKick 0.6s ease;
}
@keyframes bandKick {
  0% { box-shadow: inset 0 14px 26px -14px rgba(82, 214, 182, 0); }
  22% { box-shadow: inset 0 14px 26px -10px rgba(82, 214, 182, 0.4); }
  100% { box-shadow: inset 0 14px 26px -14px rgba(82, 214, 182, 0); }
}
.mb-radar {
  width: 58px;
  height: 58px;
...
.opt.is-pick {
  ...
  box-shadow: 0 0 0 1px var(--accent) inset, 0 6px 18px rgba(82, 214, 182, 0.16);
}
.opt.is-pick .opt-key {
  background: var(--accent);
  color: #071913;
...
@keyframes finishPulse {
  0%, 100% { box-shadow: 0 0 0 0 rgba(82, 214, 182, 0.3); }
  50% { box-shadow: 0 0 0 14px rgba(82, 214, 182, 0); }
}
.finish h2 {
  font-size: 22px;
  color: var(--ink);
