<template>
  <div class="hp tasks">
    ...
    <header class="hp-status rise" style="animation-delay: 40ms">
      <span class="hp-live"><i></i> 采样中</span>
      <span class="hp-dev mono">EMO · TASKS</span>
      <span class="hp-sig"><Icon icon="mdi:cpu-64-bit" /></span>
    </header>
    ...
    <section class="hp-stage rise" style="animation-delay: 130ms">
      <div class="hp-stage-head">
        <span class="mono">后台进程实时占用</span>
        <span class="mono hp-stage-code">PID · MIND</span>
      </div>
      <TaskMonitor />
    </section>

    <div class="hp-stats">
      <div class="hp-stat rise" style="animation-delay: 230ms"><b class="mono">{{ statA }}</b><em>监测进程</em></div>
      <div class="hp-stat rise" style="animation-delay: 290ms"><b class="mono">{{ statB }}</b><em>采样项</em></div>
      <div class="hp-stat rise" style="animation-delay: 350ms"><b class="mono">100</b><em>指数满程</em></div>
    </div>
    ...
    <main class="hp-hero">
      ...
      <p class="rise" style="animation-delay: 560ms">
        20 个日常瞬间，查看哪些念头一直在后台占用资源。不是说你有问题，是帮你看见自己被谁拖慢。
      </p>
    </main>
...
.hp-orb { position: absolute; top: -90px; right: -70px; width: 280px; height: 280px; border-radius: 50%;
  background: radial-gradient(circle, color-mix(in srgb, var(--accent) 22%, transparent), transparent 65%); pointer-events: none; z-index: 0; }
...
.hp-stage { background: var(--bg-panel); border: 1px solid var(--line); border-radius: 20px; padding: 6px; box-shadow: 0 14px 40px rgba(0, 0, 0, 0.32); }
...
.hp-cta { position: relative; width: 100%; height: 60px; border: none; border-radius: 18px; background: var(--accent); color: #1a1a17; font-size: 17px; font-weight: 900; display: flex; align-items: center; justify-content: center; gap: 10px; margin-top: 26px; overflow: hidden; transition: transform 0.4s cubic-bezier(0.32, 0.72, 0, 1); }
.hp-cta:active { transform: scale(0.975); }
.hp-cta-ring { position: absolute; left: 50%; top: 50%; width: 16px; height: 16px; border-radius: 50%; background: #fff; transform: translate(-50%, -50%); opacity: 0.4; animation: hp-breathe 2.6s ease-out infinite; }
...
.hp-cta-ic { width: 34px; height: 34px; border-radius: 50%; background: rgba(0, 0, 0, 0.16); display: grid; place-items: center; font-size: 20px; transition: transform 0.4s cubic-bezier(0.32, 0.72, 0, 1); }
