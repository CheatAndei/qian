<template>
  <main class="result shell" v-if="report">
    ...
    <section class="cta-panel">
      ...
      <button class="primary-btn" @click="openScan('interpret')">
        <span>扫码预约解读</span>
        <b aria-hidden="true">⌁</b>
      </button>
    </section>

    <button class="ghost-btn next-action-btn" @click="openScan('student')">
      继续做学生测试
    </button>
    ...
    <div class="tail-actions">
      ...
      <button class="ghost-btn" disabled>家长小孩和谐度测试 · 即将上线</button>
    </div>
    ...
    <div v-if="showScan" class="scan-overlay" @click.self="showScan = false">
      <div class="scan-modal">
        ...
        <h2>添加果果老师微信</h2>
        <p v-if="scanType === 'student'">获取学生测试链接</p>
        <p v-else>扫码添加好友，把测试结果发给老师，领取一次学习问题解读。</p>
